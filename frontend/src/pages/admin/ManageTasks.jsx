        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={col.label} className={`${col.bg} border rounded-2xl p-4`}>
              <h3 className="font-bold text-slate-800 mb-3 text-sm">{col.label} ({col.items.length})</h3>
              <div className="space-y-3">
                {col.items.map(t => (
                  <div key={t._id} className="bg-white rounded-xl p-3.5 border border-slate-100 shadow-sm">
                    <p className="text-sm font-bold text-slate-900 mb-0.5">{t.title}</p>
                    <p className="text-xs text-slate-500 mb-2">{t.client?.name} • {t.period}</p>
                    ...
                    <p className="text-xs text-slate-400 mt-2">Due: {new Date(t.deadline).toLocaleDateString("en-IN")}</p>
                    {t.accountant && <p className="text-xs text-indigo-600 mt-1">→ {t.accountant.name}</p>}
                  </div>
                ))}
                {col.items.length === 0 && <p className="text-center text-xs text-slate-400 py-4">No tasks</p>}
              </div>
            </div>
          ))}
        </div>
