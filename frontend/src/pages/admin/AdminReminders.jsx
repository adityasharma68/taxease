  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[var(--bg-surface)] rounded-2xl p-5 border border-[var(--border-subtle)]">
          ...
          <div className="text-2xl font-bold text-[var(--text-primary)]">{clients.length}</div>
          <div className="text-sm text-[var(--text-secondary)]">Email Subscribers</div>
        </div>
        <div className="bg-[var(--bg-surface)] rounded-2xl p-5 border border-[var(--border-subtle)]">
          ...
          <div className="text-2xl font-bold text-[var(--text-primary)]">{Math.floor(clients.length * 0.7)}</div>
          <div className="text-sm text-[var(--text-secondary)]">WhatsApp Opted-in</div>
        </div>
        <div className="bg-[var(--bg-surface)] rounded-2xl p-5 border border-[var(--border-subtle)]">
          ...
          <div className="text-2xl font-bold text-[var(--text-primary)]">{Math.floor(clients.length * 0.4)}</div>
          <div className="text-sm text-[var(--text-secondary)]">SMS Opted-in</div>
        </div>
        <div className="bg-[var(--bg-surface)] rounded-2xl p-5 border border-[var(--border-subtle)]">
          ...
          <div className="text-2xl font-bold text-[var(--text-primary)]">{schedule.filter(r=>r.active).length}</div>
          <div className="text-sm text-[var(--text-secondary)]">Active Reminders</div>
        </div>
      </div>
      ...
      <Card className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="font-bold text-[var(--text-primary)]">Global Channel Settings</h3>
        </div>
          ...
          {Object.entries(globalSettings).map(([channel, enabled]) => {
            ...
            return (
              <div key={channel} className={`flex items-center gap-3 px-5 py-3 rounded-xl border-2 transition-all cursor-pointer
                ${enabled ? "border-indigo-300 bg-indigo-50" : "border-[var(--border-subtle)] bg-[var(--bg-surface-2)]"}`}
                onClick={() => {
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">{enabled ? "Enabled" : "Disabled"}</p>
                </div>
                {/* Toggle switch */}
                <div className={`w-10 h-5 rounded-full transition-colors ml-2 relative ${enabled ? "bg-indigo-600" : "bg-[var(--border-default)]"}`}>
                  <div className={`absolute top-0.5 w-4 h-4 bg-[var(--bg-surface)] rounded-full shadow transition-transform ${enabled ? "translate-x-5" : "translate-x-0.5"}`} />
                </div>
              </div>
            );
          })}
      ...
      <Card>
        <CardHeader title="Reminder Schedule" action={
          <span className="text-xs text-[var(--text-muted)]">Auto-sent {schedule.filter(r=>r.active).length} days before each deadline</span>
        } />
        <div className="divide-y divide-[var(--divide-color)]">
          {schedule.map(r => (
            <div key={r.id} className="flex items-center justify-between px-5 py-4">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${r.active ? "bg-indigo-50" : "bg-[var(--bg-surface-2)]"}`}>
                  <Bell size={18} className={r.active ? "text-indigo-600" : "text-[var(--text-muted)]"} />
                </div>
                <div>
                  <p className="text-sm font-bold text-[var(--text-primary)]">{r.event}</p>
                  <p className="text-xs text-[var(--text-muted)]">Due: {r.dueDay} • Send {r.daysBefore} days before</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => toggleReminder(r.id)}
                  className={`w-12 h-6 rounded-full transition-colors relative ${r.active ? "bg-indigo-600" : "bg-[var(--border-default)]"}`}>
                  <div className={`absolute top-0.5 w-5 h-5 bg-[var(--bg-surface)] rounded-full shadow transition-transform ${r.active ? "translate-x-6" : "translate-x-0.5"}`} />
                </button>
                {/* Manual send */}
                <button
                  onClick={() => sendManualReminder(r.event)}
                  disabled={sending === r.event || !r.active}
                  className="flex items-center gap-1.5 text-xs bg-[var(--bg-surface-2)] hover:bg-indigo-100 text-[var(--text-secondary)] hover:text-indigo-700 px-3 py-2 rounded-lg font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {sending === r.event
      ...
      {/* ── Recent Reminder Log ────────────────────────────────────── */}
      <Card className="p-6">
        <h3 className="font-bold text-[var(--text-primary)] mb-4">Recent Reminder Log</h3>
        <div className="space-y-3">
          {[
            ...
            return (
              <div key={i} className="flex items-center gap-3 p-3 bg-[var(--bg-surface-2)] rounded-xl">
                <div className={`w-7 h-7 ${cfg.color} rounded-lg flex items-center justify-center flex-shrink-0`}>
                  <cfg.icon size={13} />
                </div>
                <div className="flex-1">
                  <p className="text-sm text-[var(--text-primary)] font-medium">{log.msg}</p>
