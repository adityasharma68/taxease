    ...
    <nav className="fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-lg font-bold text-slate-900" style={{fontFamily:"serif"}}>TaxEase</span>
            <span className="text-xs text-slate-400 ml-1">by WhiteCircle Group</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/login")} className="text-sm font-semibold text-slate-700 hover:text-indigo-600 px-4 py-2">Login</button>
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-bold text-slate-900">Enquire: {service}</h3>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <textarea rows={3} placeholder="Any specific requirements..."
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm resize-none focus:border-indigo-500 outline-none"
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white pt-16">
        <div className="text-center max-w-5xl mx-auto px-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            ...
            <button onClick={() => navigate("/login")} className="px-8 py-4 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-2xl border border-white/20 text-lg">
              Login to Dashboard
            </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-sm">
            {["GST Filing","ITR Filing","TDS Filing","Business Registration","Trademark","MSME"].map(s => (
              <button key={s} onClick={() => setServiceForm(s)}
                className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl border border-white/20 transition-all text-white/80 hover:text-white">
                {s}
              </button>
            ))}
          </div>
          <div className="flex items-center justify-center gap-10 mt-16 text-slate-400 text-sm">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-slate-900 mb-4" style={{fontFamily:"serif"}}>Our Services</h2>
            <p className="text-slate-500 text-lg">Complete tax and business compliance — all under one roof.</p>
          </div>
          {/* Tab switcher */}
          <div className="flex justify-center gap-2 mb-10 flex-wrap">
            {[["tax","Tax & Compliance"],["business","Business Services"],["advanced","Advanced Services"]].map(([key,label]) => (
              <button key={key} onClick={() => setActiveServiceTab(key)}
                className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeServiceTab===key?"bg-indigo-600 text-white":"bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          {activeServiceTab === "tax" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {TAX_SERVICES.map(svc => (
                <div key={svc.title} className="group p-5 border border-slate-100 rounded-2xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
          ...
          {activeServiceTab === "business" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {BUSINESS_SERVICES.map(svc => (
                <div key={svc.title} className="p-5 border border-slate-100 rounded-2xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
          ...
          {activeServiceTab === "advanced" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {ADVANCED_SERVICES.map(svc => (
                <div key={svc.title} className="p-6 border border-slate-100 rounded-2xl hover:shadow-lg transition-all bg-gradient-to-br from-white to-indigo-50/30">
                  ...
                  <p className="text-slate-500 text-sm">Enterprise-grade {svc.title.toLowerCase()} by our expert team.</p>
      ...
      <section className="py-20 bg-slate-900">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400 text-lg">Everything you need — visible at a glance.</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              <div key={item.label} className="bg-slate-800 rounded-2xl p-5 text-center border border-slate-700 hover:border-indigo-500 transition-all group">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-slate-900 mb-4" style={{fontFamily:"serif"}}>Why Choose WhiteCircle TaxEase?</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              <div key={f.title} className="flex gap-3 p-5 bg-slate-50 rounded-2xl hover:bg-indigo-50 transition-colors">
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-slate-900 mb-4" style={{fontFamily:"serif"}}>Simple, Transparent Pricing</h2>
            <p className="text-slate-500 text-lg">No hidden fees. Cancel anytime.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {PLANS.map(plan => (
              <div key={plan.name} className={`p-8 rounded-3xl border ${plan.highlighted?"border-indigo-600 bg-indigo-600 text-white shadow-xl scale-105":"border-slate-200 bg-white"}`}>
                {plan.highlighted && <div className="text-xs font-bold bg-white text-indigo-600 px-3 py-1 rounded-full inline-block mb-4">MOST POPULAR</div>}
                ...
                <div className="flex items-end gap-1 mb-6">
                  ...
                  <span className={`text-sm mb-1.5 ${plan.highlighted?"text-indigo-200":"text-slate-500"}`}>{plan.period}</span>
                </div>
                <ul className="space-y-3 mb-8">
                  {plan.features.map(f => (
                    <li key={f} className={`flex items-center gap-2.5 text-sm ${plan.highlighted?"text-indigo-100":"text-slate-600"}`}>
                ...
                <button onClick={() => setServiceForm(`${plan.name} — Enquiry`)}
                  className={`w-full py-3.5 rounded-xl font-semibold text-sm transition-all ${plan.highlighted?"bg-white text-indigo-600 hover:bg-indigo-50":"bg-indigo-600 text-white hover:bg-indigo-700"}`}>
                  Get Started
                </button>
              </div>
            ))}
      ...
      <section className="py-24 bg-slate-900 text-white">
        <div className="max-w-5xl mx-auto px-6 text-center">
          ...
          <p className="text-slate-400 mb-12">Trusted by 50,000+ businesses across India.</p>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-4xl font-bold text-slate-900 mb-12 text-center" style={{fontFamily:"serif"}}>Frequently Asked Questions</h2>
            ...
            {FAQS.map((faq,i) => (
              <div key={i} className="border border-slate-200 rounded-xl overflow-hidden">
                <button onClick={() => setActiveFaq(activeFaq===i?null:i)}
                  ...
                  {activeFaq===i ? <ChevronUp size={16} className="text-indigo-600 flex-shrink-0"/> : <ChevronDown size={16} className="text-slate-400 flex-shrink-0"/>}
                </button>
                {activeFaq===i && <div className="px-5 pb-5 text-slate-600 text-sm leading-relaxed border-t border-slate-100 pt-4">{faq.a}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-indigo-50">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-slate-900 mb-4" style={{fontFamily:"serif"}}>Careers at WhiteCircle</h2>
            <p className="text-slate-500 text-lg">Join our growing team of tax professionals and technology experts.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={job.title} className="bg-white rounded-2xl p-6 border border-slate-100 hover:shadow-md transition-shadow">
      ...
      <section className="py-20 bg-indigo-600">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button onClick={() => navigate("/register")} className="px-10 py-5 bg-white text-indigo-600 font-bold rounded-2xl hover:bg-indigo-50 transition-all text-lg hover:scale-105">
