      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" style={{ height: "70vh" }}>
        ...
        <Card className="overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-[var(--border-subtle)]">
            <h3 className="font-bold text-[var(--text-primary)] text-sm">My Clients</h3>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="flex justify-center pt-6"><Spinner /></div>
            ) : clients.length === 0 ? (
              <p className="text-center text-[var(--text-muted)] text-sm p-6">No clients assigned yet.</p>
            ) : (
              clients.map(c => (
                <button
                  key={c._id}
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors border-b border-slate-50
                    ${selectedClient?._id === c._id ? "bg-indigo-50 border-l-2 border-l-indigo-600" : "hover:bg-[var(--bg-surface-2)]"}`}
                >
